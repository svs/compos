defmodule Compos.Ui.LocalImageTest do
  use ExUnit.Case
  ...
  alias Compos.Ui.{EditorLive, LocalImage}
  ...
  test "a signed local image URL returns the image bytes" do
    path =
      Path.join(System.tmp_dir!(), "compos-local-image-#{System.unique_integer([:positive])}.png")
