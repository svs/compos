defmodule Compos.KeymapSchemeTest do
  @moduledoc """
  Runs priv/tests/keymap-test.scm alone: the key mechanism on dummy keys.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "keymap-test.scm"])
  ...
  @tag timeout: 120_000
  test "keymap-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
