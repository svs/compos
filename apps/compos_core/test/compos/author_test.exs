defmodule Compos.AuthorTest do
  use ExUnit.Case
  ...
  test "the caller process's :compos_edit_author is picked up" do
    b = new_buf()
    Process.put(:compos_edit_author, "agent:x")
    :ok = Buffer.append(b, "abc", source: :editor)
    Process.delete(:compos_edit_author)
  ...
  test "Scheme can produce line-numbered source without a core primitive" do
    path = Path.join(System.tmp_dir!(), "compos-numbered-#{System.unique_integer([:positive])}")
