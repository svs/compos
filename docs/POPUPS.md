...
10. A preview takes no focus: `popup-show-quietly` sets the popup window's buffer in place and moves the selection nowhere (a new popup is split, filled, and the selection put back in one step). A peek shows that way. `popup-show-on` selects the popup; a listing you open to work in uses it.
