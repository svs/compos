defmodule Aimax.GitTest do
  @moduledoc """
  Aimax.Core.Git against a real repository in a temporary directory.
  ...
  alias Aimax.Core.{Buffer, Git, Session}

  @twelve_lines Enum.map_join(1..12, fn n -> "line #{n}\n" end)

  setup do
    dir = Path.join(System.tmp_dir!(), "aimax-git-#{System.unique_integer([:positive])}")
  ...
  test "root reports a directory that is not a repository" do
    outside = Path.join(System.tmp_dir!(), "aimax-git-none-#{System.unique_integer([:positive])}")
