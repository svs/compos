...
(deftest 'writing-presets-are-applied-and-existing-presets-are-restored
  ...
  (lambda ()
    (let ((saved writing-presets)
      ...
      (buffer-set-local! buf 'chat-presets '(project aimax))
      (set! writing-presets '(aimax web))
      (switch-to-buffer! buf)
      (run-command "write")

      (let ((scratch (string-append "*scratch:" buf "*")))
        (check-equal! (buffer-local scratch 'chat-presets) '(aimax web project)
        ...
        (check-equal! (buffer-local scratch 'chat-presets) '(aimax research project)
                      "the removed preset is gone")

        (run-command "write")
        (check-equal! (buffer-local buf 'chat-presets) '(project aimax)
