defmodule Compos.Core.Endpoint do
  @moduledoc """
  ...
  def start(name, spec) when is_binary(name) and is_map(spec) do
    cond do
      ...
      true ->
        DynamicSupervisor.start_child(Compos.Core.EndpointSupervisor, {Conn, {name, spec}})
    end
  end
  ...
  def whereis(name) do
    case Registry.lookup(Compos.Core.EndpointRegistry, name) do
      [{pid, _}] -> pid
      [] -> nil
    end
  end
  ...
  def connections do
    Registry.select(Compos.Core.EndpointRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
  ...
  def remember(name, record), do: :persistent_term.put({:compos_endpoint_last, name}, record)

  def last(name), do: :persistent_term.get({:compos_endpoint_last, name}, nil)
end
