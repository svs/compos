defmodule Aimax.Core.WebServer do
  @moduledoc """
  ...
  @registry Aimax.Core.WebServerRegistry
  @supervisor Aimax.Core.WebServerSupervisor
  ...
  defmodule Plug do
    ...
    def call(conn, name) do
      case Aimax.Core.WebServer.read_request(conn, name) do
        {:ok, request, conn} ->
          name
          |> Aimax.Core.WebServer.dispatch(request)
          |> send_response(conn)
