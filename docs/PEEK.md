...
3. A peek is read-only: a look changes nothing, and a stray key changes nothing. Keep is one buffer-local going away: `RET` again on the row, `M-x keep-buffer`, or a change from outside the keyboard (an agent's edit). Kept, the buffer gets back the read-only state it had and keeps its window.
4. `q` dismisses a peek (the read-only keymap binds it) and puts the layout back: the popup is dismissed, a split closes, a lone window falls to the next buffer. A dropped peek leaves a row in recent; the switcher lists recent below the live buffers and hides live peeks.
