#!/usr/bin/env python3
"""Black-box dogfood runner for compos.
...
DEFAULT_RESULTS = Path(os.environ.get("COMPOS_BENCH_HOME", "~/.compos-bench")).expanduser()
DEFAULT_DAEMON_HOME = Path("/tmp/compos-bench-home")
...
def find_scenario(catalog: dict, task_id: str) -> dict:
    for task in catalog["tasks"]:
        if task["id"] == task_id:
            return task
    raise BenchError(f"unknown scenario {task_id!r}; run `bench/compos-bench list`")
...
<title>compos core acceptance suite</title><style>
...
</style></head><body><main><header><div><div class="eyebrow">compos · internal dogfood</div><h1>Core acceptance suite</h1><p class="muted">Six distinct workflows, scored on outcomes rather than leaderboard comparability.</p></div><div class="summary">{passing} / {len(CORE_SUITE)} passing</div></header>
...
def daemon_start(home: Path, project: Path, port: int, app_port: int, timeout: float) -> Rpc:
    ...
    environment.update({
        "COMPOS_HOME": str(home),
        "COMPOS_PORT": str(port),
        "COMPOS_APP_PORT": str(app_port),
        "COMPOS_NAME": "dogfood",
    })
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--home",
        type=Path,
        default=Path(os.environ.get("COMPOS_HOME", str(DEFAULT_DAEMON_HOME))).expanduser(),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "list":
        ...
        if args.json:
            ...
            print(f"compos dogfood - {result['runs']} finished runs")
