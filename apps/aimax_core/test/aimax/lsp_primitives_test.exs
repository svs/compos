defmodule Aimax.LSPPrimitivesTest do
  @moduledoc """
  ...
  alias Aimax.Core.{Buffer, LSP, Session}
  ...
  defp connected?(name) do
    case LSP.whereis(name, @root) do
      nil -> false
      pid -> Aimax.Core.LSP.Conn.status(pid) == :ready
    end
  end

  defp doc!(id, text) do
    buf = "/lsp-prim-#{System.unique_integer([:positive])}.ex"
    {:ok, _} = Aimax.Core.create_buffer(buf, text: text)
  ...
  test "the event pipe carries status, diagnostics with byte offsets, and sync echoes" do
    ...
    assert event_methods() =~ "aimax/status"
  end
