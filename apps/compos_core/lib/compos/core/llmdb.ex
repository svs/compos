defmodule Compos.Core.LLMDb do
  ...
  appended to `~/.compos/llm-usage.jsonl` (the durable ledger `report/0`
  ...
  def init(nil) do
    ...
    if Application.get_env(:compos_core, :llmdb_auto, true) do
      if stale?(), do: refresh()
  ...
  def handle_cast(:refresh, state) do
    Task.Supervisor.start_child(Compos.Core.TaskSupervisor, fn -> do_refresh() end)
    {:noreply, state}
  end
  ...
  defp db_path, do: Path.join(Compos.Core.home(), "llmdb.json")
  ...
  defp load_db do
    with {:ok, data} <- File.read(db_path()),
      ...
      :persistent_term.put(:compos_llmdb, db)
    else
      _ -> :ok
    end
  end
