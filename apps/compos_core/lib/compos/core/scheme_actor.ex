defmodule Compos.Core.SchemeActor do
  @moduledoc """
  ...
  alias Compos.Scheme
  alias Compos.Scheme.{Env, Eval}

  @registry Compos.Core.SchemeActorRegistry
  @supervisor Compos.Core.SchemeActorSupervisor
  ...
  def current, do: Process.get(:compos_scheme_actor) || false
  ...
  def init({ref, snapshot, behavior, initial_state}) do
    Process.put(:compos_scheme_actor, ref)
    Process.put(:compos_scheme_host_guard, &guard_host_primitive/2)
