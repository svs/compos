defmodule Compos.Rpc.ListenerRestartTest do
  @moduledoc """
  ...
  defp connect do
    path = Compos.Rpc.Server.default_socket_path()
  ...
  test "the rpc listener reads as up with its socket path" do
    ...
    assert rpc.address == Compos.Rpc.Server.default_socket_path()
  end

  test "restart_listener bounces the server and the socket answers again" do
    old_pid = Process.whereis(Compos.Rpc.Server)
    assert is_pid(old_pid)

    assert :ok = Daemon.restart_listener("rpc")

    new_pid = Process.whereis(Compos.Rpc.Server)
