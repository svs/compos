defmodule Compos.Core.BufferHistory do
  @moduledoc """
  A buffer's Loro document: its history, its undo stacks, and its cursors.
  See `docs/PROVENANCE-CRDT.md`.

  **Byte offsets** throughout, matching `Compos.Core.Rope` and tree-sitter.
  ...
  Unlike `Compos.Core.Rope`, a document is a mutable handle. Every edit mutates
  the same resource, so a document cannot be an undo snapshot.
  """

  alias Compos.Core.BufferHistoryNif
  ...
  **A home is a replica.** The id lives in `~/.compos/peer-id` beside the
  ...
  defp read_or_create_peer do
    path = Path.join(Compos.Core.home(), "peer-id")
