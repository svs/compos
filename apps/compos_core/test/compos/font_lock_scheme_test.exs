defmodule Compos.FontLockSchemeTest do
  @moduledoc """
  Runs priv/tests/font-lock-test.scm alone: font-lock keywords, lazy highlight, hl-line-mode.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "font-lock-test.scm"])
  ...
  @tag timeout: 120_000
  test "font-lock-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
