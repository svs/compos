...
(define *setup-secret-backends*
  '(("Doppler" "doppler" "doppler login")
    ...
    ("Linux Secret Service" "secret-tool" "secret-tool search service aimax")))
...
(define (setup--store-openrouter-key! destination key)
  (cond
    ...
    (else
      (let ((path (string-append (aimax-config-dir) "/openrouter-key")))
...
(define (setup-report)
  ...
  (let* ((backends (setup-secret-backends))
    ...
    (string-append
      ...
      "## Secret backends\n\n"
      "The key chain reads environment variables, ~/.aimax key files, then Doppler.\n"
...
(define-command "setup-report" "Open the secret-free setup status report"
  (lambda () (setup--show-document! "ai-max setup" (setup-report))))
...
(define-command "setup-bootstrap-gemini" "Set Gemini Nano as the default local connector"
  (lambda ()
    ...
    (setup--show-document! "Gemini Nano" (string-append
      "# Gemini Nano\n\n"
      "[Back to setup](aimax:setup/report)\n\n"
...
(define-command "setup-teach-keys" "Teach M-x and the ai-max philosophy"
  (lambda ()
