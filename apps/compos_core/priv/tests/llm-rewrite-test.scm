...
(deftest 'the-fence-line-is-just-a-diff-and-its-view
  "no instruction, no keys: the record carries those, the fence says diff"
  (lambda ()
    (check-equal! (llm-rewrite-theirs-block "new text")
                  "```diff theirs\nnew text\n```"
                  "theirs: the rewrite alone")
    (check-equal! (llm-rewrite--body (llm-rewrite-theirs-block "new text"))
                  "new text"
...
(deftest 'the-ours-view-is-the-passage-alone
  ...
  (lambda ()
    (check-equal! (llm-rewrite-ours-block "Two.")
                  "```diff ours\nTwo.\n```"
...
(deftest 'a-rewrite-lands-below-the-passage
  ...
  (lambda ()
    ...
    (check-equal! (buffer-text t--rw-buf)
                  (t--rw-doc (llm-rewrite-theirs-block "Deux."))
...
(deftest 'the-key-cycles-theirs-all-ours
  ...
  (lambda ()
    ...
    (check-equal! (buffer-text t--rw-buf)
                  (t--rw-doc (llm-rewrite-diff-block "Two." "Deux."))
                  "first all: the unified diff")
    (llm-rewrite-cycle-view! t--rw-buf)
    (check-equal! (buffer-text t--rw-buf)
                  (t--rw-doc (llm-rewrite-ours-block "Two."))
...
(deftest 'asking-again-rewrites-the-block-not-the-passage
  ...
  (lambda ()
    ...
    (check-equal! (buffer-text t--rw-buf)
                  (t--rw-doc (llm-rewrite-theirs-block "Zwei."))
