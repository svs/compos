...
1. A peek is a display of category `preview` (docs/DISPLAY-BUFFER.md). By the stock rule it shows in the popup, on the side away from the window it was asked from: it never covers the listing. A rule of your own, `(add-display-rule! '(category preview) 'pop-up-window)`, sends it through the window chain instead: the next peek takes the same window, and dismissing it removes the window. A peek takes no focus: it is shown by setting the popup window's buffer in place, the selection moves nowhere, and `other-window` and windmove pass its window by (`M-<down>` scrolls it; `RET` on its row opens it, and only then is it a window you can enter). A peek is a look, and the popup is where a look goes; the windows stay as they are. A popup buffer of its own (the messages) waits under the peek and comes back when the peek goes.
...
8. `M-<down>` and `M-<up>` from the listing scroll the peek: `scroll-other-window` reads the popup beside your work (a peek, the messages, the telemetry) before the next window. `scroll-popup` and `scroll-popup-down` scroll the popup by name, for a binding of your own.
