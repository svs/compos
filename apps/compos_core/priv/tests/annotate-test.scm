...
(deftest 'annotations-of-a-file-buffer-live-in-a-file-and-load-back
  ...
  (lambda ()
    (let ((fbuf (string-append (compos-home) "/zz-annotate-notes.txt")))
...
(deftest 'the-store-keeps-reader-and-llm-annotations-never-checker-ones
  ...
  (lambda ()
    (let ((fbuf (string-append (compos-home) "/zz-annotate-checked.txt")))
      (write-file! fbuf "alpha beta\n")
      (test-buffer! fbuf "alpha beta\n")
...
(deftest 'a-file-inside-a-project-stores-its-annotations-in-the-project
  ...
  (lambda ()
    (let* ((root (string-append (compos-home) "/zz-annotate-repo"))
      ...
      (check-equal! (annotate-store-file fbuf)
                    (string-append root "/.compos/annotations/src%2Fx.txt.scm")
      ...
      (check-contains! (read-file (string-append root "/.compos/annotations/src%2Fx.txt.scm"))
