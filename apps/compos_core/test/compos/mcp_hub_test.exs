defmodule Compos.MCPHubTest do
  @moduledoc """
  ...
  describe "the hub buffer" do
    setup do
      ...
      on_exit(fn ->
        ...
        Enum.each(["*mcp-hub*", "*mcp: zzhub*", "*mcp-log: zzhub*"], &Compos.Core.kill_buffer/1)
      end)
    ...
    defp open_hub_on_zzhub do
      ...
      # header, column names, then rows: 'compos first, ours last — and the
    ...
    test "a restored detail buffer comes back live, not as a frozen screenshot" do
      ...
      on_exit(fn -> Compos.Core.kill_buffer("*mcp: zzstale*") end)
    end
    ...
    test "a server never started says so instead of opening an empty detail" do
      ...
      assert eval!(~s{(begin (list-refresh! "*Messages*") (buffer-text "*Messages*"))}) =~ "has never been started"
    end
  end
end
