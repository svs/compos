...
5. `q` on the peek dismisses it (the read-only keymap binds it). `q` in dired dismisses the peek that shows, and with none showing leaves dired. A dismissed peek leaves a row in recent; the switcher lists recent below the live buffers and hides live peeks.
