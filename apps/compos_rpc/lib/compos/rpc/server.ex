defmodule Compos.Rpc.Server do
  @moduledoc """
  JSON-RPC 2.0 over a Unix domain socket, newline-delimited — the compos
  ...
  def default_socket_path do
    Application.get_env(:compos_rpc, :socket_path, Path.expand("~/.compos/sock"))
  end
  ...
  def init(opts) do
    ...
    Logger.info("compos rpc listening on #{path}")
  ...
  defp accept_loop(lsock, server) do
    case :gen_tcp.accept(lsock) do
      {:ok, sock} ->
        {:ok, pid} = Task.Supervisor.start_child(Compos.Rpc.ConnSupervisor, fn -> serve(sock) end)
