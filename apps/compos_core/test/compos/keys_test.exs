defmodule Compos.KeysTest do
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      File.rm(Path.join(Compos.Core.home(), "zz_keys_file-key"))
      eval!("(key-forget-all!)")
    end)
  ...
  test "a file under ~/.compos answers when the environment does not" do
    File.write!(Path.join(Compos.Core.home(), "zz_keys_file-key"), "from-file\n")
