defmodule Aimax.Core.LSP do
  ...
  stored in :aimax_escaped_closures like the MCP handler): the handler
  receives (ID METHOD PARAMS). Status changes arrive on the same pipe
  as method "aimax/status". Callbacks run on the connection's own
  `{:lsp, key}` lane, so a diagnostics burst never queues behind a
  keystroke.
  """

  alias Aimax.Core.{Session, LLM}
  alias Aimax.Core.LSP.Conn
  ...
  def start(name, root, spec) when is_binary(name) and is_binary(root) do
    cond do
      ...
      true ->
        DynamicSupervisor.start_child(Aimax.Core.LSPSupervisor, {Conn, {{name, root}, spec}})
    end
  end
  ...
  def whereis(name, root) do
    case Registry.lookup(Aimax.Core.LSPRegistry, {name, root}) do
      [{pid, _}] -> pid
      [] -> nil
    end
  end
  ...
  def connections do
    Registry.select(Aimax.Core.LSPRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
  ...
  def notify(key, status),
    do: dispatch_event(key, "aimax/status", %{"status" => to_string(status)})
  ...
  def remember(key, record), do: :persistent_term.put({:aimax_lsp_last, key}, record)

  def last(key), do: :persistent_term.get({:aimax_lsp_last, key}, nil)
end
