defmodule Compos.Core.LLM do
  ...
  defp run_request(prompt, requested_model) do
    case Application.get_env(:compos_core, :llm_request_fun) do
  ...
  defp run_tool_blocks(blocks, dispatcher, opts) do
    ...
    if length(calls) > 1 and Enum.all?(calls, &read_tool?(&1, opts)) do
      ...
      max_concurrency = Application.get_env(:compos_core, :scheme_read_concurrency, 4)
  ...
  defp run_tool(dispatcher, name, input, concurrent?) do
    ...
    case result do
      ...
      {:ok, v} -> {Compos.Scheme.Printer.print(v), false}
      {:error, msg} -> {"error: #{msg}", true}
    end
  catch
  ...
  @doc "MCP annotations derived from one compos tool effect declaration."
  ...
  defp chat_fun do
    Application.get_env(:compos_core, :llm_chat_fun, &default_chat/1)
  end
  ...
  defp backoff_ms(attempt) do
    base = Application.get_env(:compos_core, :llm_retry_base_ms, 500) * Integer.pow(2, attempt - 1)
  ...
  defp req_opts(spec, tools) do
    ...
    extra = Application.get_env(:compos_core, :llm_req_opts, [])
  ...
  """
  def set_cache_ttl(ttl) when is_binary(ttl), do: :persistent_term.put(:compos_llm_cache_ttl, ttl)

  defp cache_ttl, do: :persistent_term.get(:compos_llm_cache_ttl, "1h")
  ...
  def set_model(model), do: :persistent_term.put(:compos_llm_model, model)

  def model do
    case :persistent_term.get(:compos_llm_model, nil) do
      nil -> Application.get_env(:compos_core, :llm_model, "claude-sonnet-5")
      m -> m
    end
  end
