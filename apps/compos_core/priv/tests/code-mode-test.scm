...
(define (t--cm-off! &rest extra)
  (for-each
    ...
    (buffer-list))
  (customize-set! 'code-presets '(compos))
...
(deftest 'code-mode-joins-a-group-loads-the-presets-and-turns-on-llm-mode
  ...
  (lambda ()
    (let ((buf (t--cm-on!)))
      ...
      (check-equal! (buffer-local buf 'chat-presets) '(compos) "with the coding presets")
      (check-equal! (buffer-local buf 'modeline-info) "code · compos" "and the modeline says so"))
    (t--cm-off!)))
...
(deftest 'the-coding-presets-ride-over-the-buffers-own
  ...
  (lambda ()
    (let ((buf (t--cm-fresh!))
      ...
      (customize-set! 'code-presets '(compos web))
      (switch-to-buffer! buf)
      (run-command "code-mode")

      (check-equal! (buffer-local buf 'chat-presets) '(compos web project)
                    "the coding presets come first, the buffer's own last")
      (check-equal! (buffer-local buf 'modeline-info) "code · compos web project" "the modeline")

      ;; a live customization rebuilds from the buffer's own pre-mode value
      (customize-set! 'code-presets '(compos))
      (check-equal! (buffer-local buf 'chat-presets) '(compos project)
...
(deftest 'the-scratch-chat-carries-the-coding-presets
  ...
  (lambda ()
    (let* ((buf (t--cm-on!))
      ...
      (check-equal! (buffer-local scratch 'chat-presets) '(compos) "with the coding presets")
...
(deftest 'a-scratch-already-open-picks-up-the-presets-when-code-mode-turns-on
  ...
  (lambda ()
    (let* ((buf (t--cm-fresh!))
      ...
      (check-equal! (buffer-local scratch 'chat-presets) '(compos) "the open scratch picked them up")
...
(deftest 'restore-minor-modes-re-runs-the-code-setup-once
  ...
  (lambda ()
    (let ((buf (t--cm-on!)))
      (buffer-set-local! buf 'chat-presets '(compos project))
      ...
      (check-equal! (buffer-local buf 'modeline-info) "code · compos" "and the modeline"))
    (t--cm-off!)))
...
(deftest 'code-mode-asks-before-it-assigns-this-frame-a-worktree
  ...
  (lambda ()
    (let* ((root (t--cm-git! (string-append (compos-home) "/zz-cm-proj")))
      ...
      (let ((task (current-buffer)))
        ...
        (check-equal! (buffer-local task 'modeline-info) "code · a1 · compos" "the modeline says both")
