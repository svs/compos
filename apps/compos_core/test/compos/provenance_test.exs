defmodule Compos.ProvenanceTest do
  @moduledoc """
  ...
  alias Compos.Core.{Buffer, BufferHistory, BufferHistoryStore, Editor, Session}

  defp unique(label), do: "*provenance-#{label}-#{System.unique_integer([:positive])}*"

  defp new_buffer(label, text \\ "") do
    name = unique(label)
    {:ok, ^name} = Compos.Core.create_buffer(name, text: text)
