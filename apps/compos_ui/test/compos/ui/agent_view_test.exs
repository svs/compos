defmodule Compos.Ui.AgentViewTest do
  use ExUnit.Case
  ...
  setup do
    ...
    on_exit(fn ->
      Enum.each(Compos.Core.list_buffers(), fn name ->
        if String.starts_with?(name, "*agent"), do: Compos.Core.kill_buffer(name)
      end)
  ...
  test "tool card highlights the call argument apart from the tool name", %{conn: conn} do
    ...
    assert has_element?(view, ".ag-title .ag-tool-name", "compos:apropos")
