defmodule Compos.GitDiffTest do
  @moduledoc """
  ...
  setup do
    dir = Path.join(System.tmp_dir!(), "compos-diff-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
  ...
  defp open_diff(ctx) do
    ...
    {:ok, _} = Compos.Core.create_buffer(scratch)
  ...
  defp line_of(buf, pos), do: Compos.Core.Text.line_index(Buffer.text(buf), pos) + 1
  ...
  test "C-x g from a repo-less buffer means the most recent repository", ctx do
    ...
    {:ok, _} = Compos.Core.create_buffer(host)
    ...
    {:ok, _} = Compos.Core.create_buffer(stray)
  ...
  test "w arms the watcher and the buffer refreshes with no keypress", ctx do
    ...
    assert ctx.root in Compos.Core.Watch.watching()
    ...
    refute ctx.root in Compos.Core.Watch.watching()
  end
