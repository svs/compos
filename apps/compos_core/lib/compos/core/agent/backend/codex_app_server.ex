defmodule Compos.Core.Agent.Backend.CodexAppServer do
  @moduledoc """
  ...
  def init({config, owner}) do
    ...
    {:ok,
     request(state, "initialize", %{
       "clientInfo" => %{"name" => "compos", "title" => "Compos", "version" => "0.1"},
       "capabilities" => %{}
     })}
  end
  ...
  defp developer_instructions(config, model, context) do
    base =
      ...
      |> case do
        nil -> nil
        meta -> meta |> Compos.Core.Plist.to_json() |> get_in(["systemPrompt", "append"])
      end
      |> string_value()
  ...
  defp emit_model_state(state) do
    available =
      for m <- state.models do
        Compos.Core.ModelCatalog.picker_entry(m)
      end
