defmodule Compos.WorktreesTest do
  ...
  defp scratch_repo do
    root = Path.join(System.tmp_dir!(), "compos-wt-#{System.os_time(:nanosecond)}")
    File.rm_rf!(root)
  ...
  setup do
    ...
    on_exit(fn ->
      Compos.Core.kill_buffer("*worktrees*")
  ...
  test "the LLM chat name becomes the human project workspace name" do
    workspace = "/tmp/compos-worktrees/a1"
    project = "/tmp/compos"
    ...
    assert owner =~ ~s{workspace-project "compos"}
    assert owner =~ ~s{workspace-name "workspace prompts"}
  after
    Compos.Core.kill_buffer("*chat:workspace-name-test*")
  end
  ...
  test "new chats inherit the workspace LLM defaults" do
    ...
    eval!("""
    (begin
      (buffer-set-local! "#{path}" 'chat-presets '(compos web))
    ...
    assert eval!(~s{(buffer-local "#{chat}" 'chat-presets)}) == "(compos web)"
  ...
  after
    ...
    |> Enum.each(&Compos.Core.kill_buffer/1)
  end
  ...
  test "the *worktrees* buffer lists, diffs, and removes" do
    ...
    assert eval!(~s{(daemon-workspace-owner "#{root}-worktrees/t3")}) =~ "compos"
  ...
  after
    Compos.Core.kill_buffer("*workspace diff: t3*")
  end
  ...
  after
    Compos.Core.kill_buffer("*workspace diff: t5*")
