---
name: compos-boot
description: Boot a second compos daemon on another port that reads the user's real config. Use whenever a task needs an extra daemon beside the running one — a scratch instance, a second workspace, a config repro, or a verify run that must load the real init.scm.
---
...
```sh
COMPOS_HOME=~/.compos-alt COMPOS_CONFIG=~/.compos \
COMPOS_PORT=4020 COMPOS_APP_PORT=4021 COMPOS_NAME=alt \
COMPOS_DAEMON_REGISTRY=~/.compos-alt/daemons.json \
  nohup mix run --no-halt >> ~/.compos-alt/daemon.log 2>&1 </dev/null &
```
...
|---|---|---|
| `COMPOS_HOME` | state: `desktop.etf`, `buffers/`, `docs/`, `sock`, `peer-id`, `llmdb.json` | the new daemon writes the user's desktop and buffers |
| `COMPOS_CONFIG` | where user Scheme reads from: `ai-config.scm`, `init.scm`, `custom.scm`, `theme.scm`, key files, `packages/` | the new daemon boots stock, with none of the user's config |
| `COMPOS_PORT` | the editor origin | the endpoint fails with `:eaddrinuse` and the whole boot aborts |
| `COMPOS_APP_PORT` | the preview-app origin | the same crash, one port later |
| `COMPOS_NAME` | the label in the extension's daemon list | it defaults to the home's basename |
| `COMPOS_DAEMON_REGISTRY` | the shared daemon list | the new daemon writes a row into the user's `~/.compos/daemons.json` |

`COMPOS_HOME` alone moves the socket and the desktop file with it. The other
paths follow the home, so `COMPOS_SOCK` and `COMPOS_DESKTOP` are only for the
...
Every knob except `COMPOS_CONFIG` also reads from `$COMPOS_HOME/daemon.conf`,
one `key = value` per line. This survives a restart, so prefer it for a
daemon that stays around:

```sh
mkdir -p ~/.compos-alt
cat > ~/.compos-alt/daemon.conf <<'EOF'
port = 4020
app_port = 4021
name = alt
registry = /Users/svs/.compos-alt/daemons.json
EOF
...
`COMPOS_CONFIG` is read from the environment only. The conf file has no key
for it. An environment variable wins over the file for every other key.
`COMPOS_CONF=/path/to/file` names a conf file outside the home.

## Choose the port pair, do not guess it

Other daemons already hold ports in the 4000s: the user's daemon on 4004/4005,
worktree daemons from 4204 up, `COMPOS_VERIFY` on 4104/4105. A taken port kills
...
for i in $(seq 1 60); do
  printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"ping","params":{}}' \
    | nc -U ~/.compos-alt/sock 2>/dev/null | grep -q pong && break
  sleep 1
done
```
...
```sh
bin/compos --home ~/.compos-alt state
bin/compos --home ~/.compos-alt eval '(list (compos-home) (compos-config-dir) (editor-url))'
# a command defined only in the user's init.scm proves the config ran
bin/compos --home ~/.compos-alt eval '(member "SOME-USER-COMMAND" (command-names))'
```

`bin/compos --home` reads the same home for `eval`, `log`, `bridge`, `keys`,
and `state`. `mix compos.reload --home ~/.compos-alt FILE` reloads there.

## Stop it

```sh
printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"shutdown","params":{}}' | nc -U ~/.compos-alt/sock
```
...
`compos_ui` sets the real URL later. So the row the daemon writes at boot
...
`COMPOS_DAEMON_REGISTRY`, or accept a wrong row in the user's list. Calling
...
`COMPOS_CONFIG`. Two daemons that point at `~/.compos` overwrite each other's
...
`apps/compos_core/priv`, and the config dir. One save reloads the user's daemon
...
a different checkout — `Compos.Core.Daemon` does that for worktree daemons.
...
- `COMPOS_VERIFY=1 mix run --no-halt` is the fixed, stock version of this: home
  `/tmp/compos-verify-home`, ports 4104/4105, socket `/tmp/compos-verify.sock`,
  and **no** user config. Use it to verify a change from a worktree. Use this
  skill when the task needs the user's real config.
- `compos-restart` covers getting a change into a daemon that already runs.
- `compos-debug` covers driving one and reading what it did.
