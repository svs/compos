...
;;;
;;; Mechanism is Compos.Core.MCP (stdio/http JSON-RPC, handshake, tool
...
(define (chat-presets-of buf)
  ...
  (let ((presets (or (buffer-local buf 'chat-presets) '())))
    (if (member 'compos presets) presets (cons 'compos presets))))
...
;; The intrinsic `compos` surface already lives in this process. ACP receives
...
(define (chat-remote-servers buf)
  (remove (lambda (s) (equal? s 'compos)) (chat-active-servers buf)))

(define (chat-compos-tools? buf)
  (and (member 'compos (chat-active-servers buf)) #t))
...
(define (chat-extra-tool-specs buf)
  (let ((servers (chat-remote-servers buf)))
    (append
      (if (and (chat-compos-tools? buf) (boundp (quote llm-tool-specs)))
          (llm-tool-specs)
          '())
      (if (null? servers)
          '()
...
(define (chat-tool-system-parts buf)
  (let* ((compos? (chat-compos-tools? buf))
         ...
         (mode-parts
           (if (boundp (quote prompt-buffer-parts))
               ...
               '()))
         (guidance
           (if (and compos? (boundp (quote compos-direct-prompt-parts)))
               (compos-direct-prompt-parts)
               '())))
    (filter
...
;;; other caller — a command, a hook, an agent through the compos proxy —
...
;; 'compos is this editor served back to itself: the proxy answers
  ...
  (let ((names (map symbol->string servers)))
    (if (null? names)
        ...
        (string-append
          ...
          "and never run a shell command to reach one. For compos editor, "
          "mail, and browser work, always use the compos MCP tools rather "
          ...
          "call the compos apropos tool once, then run the best hit's use "
...
;; mcp-register!/define-preset! in ~/.compos/ai-config.scm.
...
;; The socket is named, not defaulted: a second daemon (COMPOS_HOME) listens
;; on its own path, and its agents must reach IT, not the default one.
(mcp-register! 'compos
  (list 'command "elixir"
        'args (list (priv-path "compos-mcp-proxy.exs"))
        'env (list 'COMPOS_SOCK (compos-socket-path))))

(define-preset! 'compos "Live compos editor tools" '(compos))
...
;; the other. This line reports what compos sent, which is the part compos
;; controls.
...
;;; configuration compos sent to an ACP agent, which is what to read when a
;;; tool is missing.
...
;; chat-presets-of, which guarantees the intrinsic compos editor bridge.
