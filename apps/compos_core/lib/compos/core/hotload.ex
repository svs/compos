defmodule Compos.Core.Hotload do
  @moduledoc """
  ...
  def handle_info({:EXIT, pid, reason}, state) do
    case state.pids[pid] do
      ...
      root ->
        if reason != :normal,
          do: Logger.warning("Compos.Core.Hotload: the watcher for #{root} stopped")
  ...
  defp backend, do: Application.get_env(:compos_core, :fs_backend, FileSystem)

  defp default_roots do
    if Application.get_env(:compos_core, :hotload, false),
  ...
  # ~/.compos/packages. Editing config in the editor applies it on save.
  defp config_roots do
    dir = Compos.Core.config_dir()
  ...
  defp start_watch(root, state) do
    case backend().start_link(dirs: [root]) do
      ...
      {:error, reason} ->
        Logger.warning("Compos.Core.Hotload: cannot watch #{root}: #{inspect(reason)}")
  ...
  # config.exs names `{Compos.Core.Hotload.Compile, :compile, []}` for dev:
  ...
  defp recompile(_paths) do
    case Application.get_env(:compos_core, :hotload_recompile) do
      {m, f, a} ->
        case apply(m, f, a) do
          :ok ->
          ...
          {:error, output} ->
            Logger.error("Compos.Core.Hotload: compile failed\n#{output}")
  ...
  defp refresh_primitives do
    ...
    e -> Logger.error("Compos.Core.Hotload: rebinding primitives failed: #{Exception.message(e)}")
  catch
    :exit, _ -> :ok
  end
  ...
  defp reload_scheme(paths) do
    if Session.ready?() do
      case Session.reload_files(paths) do
        ...
        {:error, reason} ->
          Logger.error("Compos.Core.Hotload: reload failed: #{inspect(reason)}")
