...
(define (prompt-parts-changed! buf)
  ;; A direct chat reads the new value on its next turn. ACP must start a new
  ;; session before its next turn because the append is fixed at session/new.
  (when (and (boundp (quote chat-stateless?))
...
(define (chat-prompt-report buf)
  (let* ((direct? (chat-prompt-direct? buf))
    ...
    (string-append
      ...
      (if direct?
          (string-append
            "The direct lane recomposes this value for each turn. "
            "An unchanged byte prefix keeps its provider cache hit.\n\n")
          (string-append
            "ACP evaluates this append when a session starts. "
            "A live session keeps its earlier value until reconnect.\n\n"
            "This page reconstructs the append from current buffer state. "
