defmodule Compos.GroupsSchemeTest do
  @moduledoc """
  Runs priv/tests/groups-test.scm alone: groups: records, names, membership, and the frame context.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "groups-test.scm"])
  ...
  @tag timeout: 120_000
  test "groups-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
