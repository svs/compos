defmodule Compos.Ui.PreviewLinkTest do
  ...
  setup do
    ...
    root =
      Path.join(System.tmp_dir!(), "compos-preview-link-#{System.unique_integer([:positive])}")
  ...
  test "a reply action link fills the current group's chat without sending", %{
    ...
    |> render_hook("preview_link", %{
      "win" => win,
      "href" => "compos:reply/Use%20the%20safe%20option."
    })
