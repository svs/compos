defmodule Compos.CacheEconomicsTest do
  @moduledoc """
  ...
  alias Compos.Core.{Agent, Buffer, Editor, KeyDispatch, LLMDb, Session}
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:compos_core, :llm_req_opts)
      Application.delete_env(:compos_core, :llm_retry_base_ms)
      Application.delete_env(:compos_core, :llm_max_tokens)
  ...
  defp stub_chat(fun), do: Application.put_env(:compos_core, :llm_chat_fun, fun)
  ...
  test "a cancelled turn still writes its ledger row, priced and attributed" do
    ...
    ledger = Path.join(Compos.Core.home(), "llm-usage.jsonl")
    File.rm(ledger)
  ...
  test "an overloaded provider is retried, not handed back to the user" do
    Application.put_env(:compos_core, :llm_retry_base_ms, 1)
    {:ok, counter} = Agent0.start()

    Application.put_env(:compos_core, :llm_req_opts,
      req_http_options: [
        plug: fn conn ->
    ...
    assert {:ok, text} = Compos.Core.LLM.request("are you there")
