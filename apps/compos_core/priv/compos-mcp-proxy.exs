# compos-mcp-proxy — the editor's tool registry served as an MCP server.
#
# External ACP agents (claude-code, codex) get eval-scheme — and with it
# the whole editor API — through this bridge: stdio MCP
# on this side, the daemon's JSON-RPC socket (~/.compos/sock, COMPOS_SOCK to
# override) on the other. Spawned per agent session via the 'compos entry
...
defmodule ComposProxy do
  def sock, do: System.get_env("COMPOS_SOCK") || Path.expand("~/.compos/sock")
    ...
    case message do
      %{"method" => "initialize", "id" => id, "params" => params} ->
        reply(id, %{
          ...
          capabilities: %{tools: %{}},
          serverInfo: %{name: "compos", version: "0.1.0"}
        })
  ...
  defp effects_table do
    case :ets.whereis(:compos_proxy_effects) do
      :undefined -> :ets.new(:compos_proxy_effects, [:named_table, :public, :set])
      table -> table
    end
  end
  ...
  # COMPOS_AGENT (set by the ACP backend at spawn) names the thread this
  ...
  defp agent_slug do
    case System.get_env("COMPOS_AGENT") do
      nil -> nil
...
unless System.get_env("COMPOS_PROXY_NO_LOOP") == "1", do: ComposProxy.loop()
