...
4. `C-\`` toggles the popup; `C-M-\`` (`popup-bufferize`) makes it an ordinary window in the place it occupies; `C-c p` puts any buffer in the popup; `q` in a listing closes it.
5. A popup is a visit, not a place: it says nothing about the frame's group (docs/groups.md).
6. `*Messages*`, `*Telemetry*`, `*ibuffer*`, `*shell*`, `*llm*`, and `*opencode…` are popups by rule.
