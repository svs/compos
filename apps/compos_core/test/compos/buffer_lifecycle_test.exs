defmodule Compos.BufferLifecycleTest do
  use ExUnit.Case, async: false

  alias Compos.Core.{Buffer, BufferStore, Editor, Events, Session}
  ...
  test "an immutable buffer ref survives rename, eviction, and wake" do
    old = unique("ref-old")
    new = unique("ref-new")
    {:ok, ^old} = Compos.Core.create_buffer(old, text: "stable")
    ref = Buffer.ref(old)
    id = Buffer.id(ref)

    assert {:ok, ^new} = Compos.Core.rename_buffer(ref, new)
    ...
    assert :ok = Compos.Core.kill_buffer(ref)
    refute Buffer.exists?(ref)
  ...
  test "rename moves the file and carries stable buffer identity, windows, and history" do
    root = Path.join(System.tmp_dir!(), "compos-rename-#{System.unique_integer([:positive])}")
    source = Path.join(root, "old.txt")
    destination = Path.join(root, "nested/new.txt")
    File.mkdir_p!(root)
    File.write!(source, "hello")
    {:ok, ^source} = Compos.Core.open_file(source)
    Editor.set_window_buffer(source)
    id = Buffer.eviction_info(source).id

    assert {:ok, ^destination} = Compos.Core.rename_file(source, destination)
    ...
    Editor.set_window_buffer("*scratch*")
    Compos.Core.kill_buffer(destination)
    File.rm_rf!(root)
  end

  test "idle buffers leave memory but remain in history and wake on selection" do
    old = Application.get_env(:compos_core, :buffer_idle_timeout_ms)
    Application.put_env(:compos_core, :buffer_idle_timeout_ms, 30)

    on_exit(fn ->
      Application.put_env(:compos_core, :buffer_idle_timeout_ms, old || 24 * 60 * 60 * 1_000)
    end)
  ...
  test "renaming a dormant file updates its catalog without retaining a process" do
    root =
      Path.join(System.tmp_dir!(), "compos-dormant-rename-#{System.unique_integer([:positive])}")
  ...
  test "binary files open read-only and survive save and checkpoint byte-for-byte" do
    path = Path.join(System.tmp_dir!(), "compos-binary-#{System.unique_integer([:positive])}.etf")
