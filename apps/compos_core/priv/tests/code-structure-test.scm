...
(deftest 'a-headless-find-file-still-parses-with-the-grammar
  ...
  (lambda ()
    ;; compos-home, not /tmp: under mix test it is the run's own home, and
    ;; in the live editor it is ~/.compos. Either way the file is ours.
    (let ((path (string-append (compos-home) "/zz_headless_code.ex")))
