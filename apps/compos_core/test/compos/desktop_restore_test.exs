defmodule Compos.DesktopRestoreTest do
  @moduledoc """
  ...
  test "desktop-clear saves modified files and removes the cleared desktop" do
    ...
    on_exit(fn ->
      File.rm(path)
      for b <- [path, clean], Buffer.exists?(b), do: Compos.Core.kill_buffer(b)
    end)
  ...
  test "A saves every remaining file and N saves none" do
    ...
    on_exit(fn ->
      for path <- [a, b] do
        File.rm(path)
        if Buffer.exists?(path), do: Compos.Core.kill_buffer(path)
      end
    end)
  ...
  test "every literal mode-name write names a registered mode" do
    priv = to_string(:code.priv_dir(:compos_core))
  ...
  test "a restored org file comes back folded" do
    path = Path.join(System.tmp_dir!(), "compos-restore-#{System.unique_integer([:positive])}.org")
    File.write!(path, "* one\nbody one\n* two\nbody two\n")
  ...
  test "a clean buffer rewrites no checkpoint" do
    ...
    file = Compos.Core.BufferStore.checkpoint_path(Buffer.id(name))
