...
10. A peek popup — a popup that takes no focus — is transparent to the point. `popup-show-quietly` sets the popup window's buffer in place and moves the selection nowhere (a new popup is split, filled, and the selection put back in one step), and it records nothing: no return place, no work windows, no layout, so closing it restores nothing — the restores are for a popup you entered, and they carried every window's point back to the moment the popup opened. Nothing that manages point or focus sees a peek popup: `other-window` and windmove pass it by, a selection report from another window is dropped, a kill never fills a window with it, and un-floating a peek runs no mode setup. `popup-show-on` selects the popup and records the return place; a listing you open to work in uses it.
