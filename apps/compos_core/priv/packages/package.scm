...
;;; user ones live in <compos-home>/packages. Load the ones you want explicitly
;;; from ~/.compos/init.scm, after the bundled package manifest has completed.
...
(define (package-dir) (string-append (compos-home) "/packages"))
...
(category! 'packages)
(public! 'package-install! "(package-install! SPEC) — fetch a .scm (github user/repo[/path] or url) into <compos-home>/packages and load it")
...
;;;
;;; In development the daemon also does this by itself: Compos.Core.Hotload
