...
(define (pdf--document-html path page total zoom image text &optional original dark?)
  (let ((title (pdf--html-escape (pdf--basename path))))
    (string-append
      ...
      ".toolbar{position:sticky;top:0;z-index:2;padding:12px 18px 11px;border-bottom:1px solid;"
      "background:Canvas;box-shadow:0 1px 10px rgba(0,0,0,.08)}"
      ".identity{display:flex;align-items:baseline;gap:16px;min-width:0;margin-bottom:9px}"
      ".title{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:650;letter-spacing:.01em}"
      ".status{margin-left:auto;display:flex;gap:8px;white-space:nowrap;font-size:12px;opacity:.72}"
      ...
      "@media(max-width:640px){.toolbar{padding:10px}.identity{display:block}.status{margin:4px 0 0}.viewer{padding:14px}}"
      "</style></head><body" (if dark? " class=\"dark-page\"" "") "><header class=\"toolbar\">"
      "<div class=\"identity\"><span class=\"title\" title=\"" title "\">" title "</span>"
      "<span class=\"status\"><span>Page " (number->string page) " of "
      (number->string total) "</span><span>" (number->string zoom) "%</span>"
      (if dark? "<span>Dark page</span>" "") "</span></div>"
