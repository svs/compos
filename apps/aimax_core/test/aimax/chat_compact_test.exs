defmodule Aimax.ChatCompactTest do
  ...
  describe "the suggestion follows the model" do
    setup do
      ...
      on_exit(fn -> :persistent_term.put(:aimax_llmdb, db) end)
      :ok
    end

    test "LLMDb prefers the input limit over the context limit" do
      assert Aimax.Core.LLMDb.context_limit("openai:small-ctx") == 8_000
      assert Aimax.Core.LLMDb.context_limit("context-only") == 500_000
      assert Aimax.Core.LLMDb.context_limit("openai:no-such-model") == nil
    end
