defmodule Compos.HotloadTest.NoBackend do
  @moduledoc """
...
defmodule Compos.HotloadTest.Recompiler do
...
defmodule Compos.HotloadTest do
  @moduledoc """
  Compos.Core.Hotload: a saved file reaches the running daemon.
  ...
  alias Compos.Core.{Hotload, Session}

  # 200 ms debounce; give the flush room, then prove no second one follows
  @settle 1_500

  setup do
    Application.put_env(:compos_core, :fs_backend, Compos.HotloadTest.NoBackend)
    Application.put_env(:compos_core, :hotload_recompile, {Compos.HotloadTest.Recompiler, :ok, [self()]})

    on_exit(fn ->
      Application.delete_env(:compos_core, :fs_backend)
      Application.delete_env(:compos_core, :hotload_recompile)
    end)
  ...
  describe "the filter" do
    test "names the three source kinds and nothing else" do
      assert Hotload.source?("/p/apps/compos_core/priv/packages/git.scm")
      assert Hotload.source?("/p/apps/compos_ui/lib/compos/ui/editor_live.ex")
      assert Hotload.source?("/p/apps/compos_ui/lib/compos/ui/page.heex")
    ...
    test "refuses a partial write, so a reload never sees half a file" do
      ...
      refute Hotload.source?("/p/apps/compos_core/priv/.#git.scm")
      refute Hotload.source?("/p/apps/compos_core/priv/git.scm~")
      refute Hotload.source?("/p/apps/compos_core/priv/#git.scm#")
    end

    test "refuses build output and session state, which the daemon writes constantly" do
      refute Hotload.source?("/p/_build/dev/lib/compos_core/priv/editor.scm")
      refute Hotload.source?("/p/deps/phoenix/lib/phoenix.ex")
      refute Hotload.source?("/p/apps/compos_core/native/compos_ts/target/debug/build.ex")
      refute Hotload.source?("/p/.git/COMMIT_EDITMSG.ex")
      refute Hotload.source?("/home/u/.compos/buffers/scratch.scm")
      refute Hotload.source?("/home/u/.compos/chats/a/turn.scm")
    end
    ...
    test "refuses a tree-sitter query, which is Scheme only to look at" do
      ...
      refute Hotload.source?("/home/u/.compos/grammars/markdown-highlights.scm")
      refute Hotload.source?("/home/u/.compos/grammars/src/markdown/queries/highlights.scm")
    end
  end
  ...
  describe "reloading" do
    ...
    test "a compile failure is reported, and the daemon stays up" do
      Application.put_env(
        :compos_core,
        :hotload_recompile,
        {Compos.HotloadTest.Recompiler, :fail, [self()]}
      )
      ...
      assert :code.is_loaded(Compos.Core.Editor) != false
      assert :code.is_loaded(Compos.Core.Buffer) != false
    end

    # A primitive is an anonymous fun captured from Compos.Core.SchemeAPI when
  ...
  # the new one. Compos.Core.Editor died in that gap twice on 2026-08-29:
  ...
  describe "the module swap" do
    alias Compos.Core.Hotload.Compile
    ...
    test "the project ebins are the umbrella apps" do
      ebins = Compile.ebins()
      assert Enum.any?(ebins, &String.ends_with?(&1, "/compos_core/ebin"))
