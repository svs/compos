---
name: compos-debug
description: Drive a running compos daemon and read what it did — evaluate Scheme, dispatch keys, inspect frames/windows/buffers, and tail the browser bridge (both the daemon's side and the extension's forwarded errors). Use whenever debugging compos behaviour, the Chrome extension, or anything reported as "buggy" rather than reproducing by hand or asking the user to test.
---

# Debugging compos

`bin/compos` talks to a running daemon over its RPC socket and reads its log.
Reach for it before guessing, and before asking the user to reproduce anything.

```sh
bin/compos state                  # frames, windows, current buffer, prompt, browser
bin/compos eval '(buffer-list)'   # any Scheme; the whole editor is reachable
bin/compos keys C-x b             # dispatch keys as if typed, then show the prompt
bin/compos bridge -n 60           # every browser op, both directions
bin/compos log -f                 # the daemon log with LiveView noise stripped
bin/compos tabs                   # what the browser last told us is open
```

`--home` (or `COMPOS_HOME`) picks the daemon; it defaults to `~/.compos-web`.
The one on `~/.compos` is usually the user's real editor — prefer a separate
daemon for experiments:

```sh
COMPOS_HOME=~/.compos-web COMPOS_PORT=4005 COMPOS_NAME=web mix run --no-halt
```
...
5. assert with `bin/compos eval` on the editor's actual state

## What the extension can and can't tell you

- **Can**: its errors and warnings (forwarded), which daemons it is connected
  to, which tab is registered as each window's compos frame.
...
- **A prompt left open persists in `desktop.etf`** and survives a restart.
  `bin/compos eval '(when (minibuffer-state) (minibuffer-cancel!))'`.
