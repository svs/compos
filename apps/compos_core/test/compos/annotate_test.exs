defmodule Compos.AnnotateTest do
  @moduledoc """
  ...
  setup do
    ...
    on_exit(fn ->
      for b <- [@list, @buf] do
        if Buffer.exists?(b), do: Compos.Core.kill_buffer(b)
      end
    end)
  ...
  test "a margin card click hands the focus back to the document" do
    ...
    on_exit(fn -> if Buffer.exists?("*margin*"), do: Compos.Core.kill_buffer("*margin*") end)
    ...
    Compos.Core.SchemeAPI.block_click("*margin*", "ann:pick:#{id}")
