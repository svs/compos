defmodule Compos.MorgTodosTest do
  use ExUnit.Case
  ...
  setup do
    dir = Path.join(System.tmp_dir!(), "compos-morg-todos-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
    ...
    on_exit(fn ->
      ...
      if Buffer.exists?(@todos), do: Compos.Core.kill_buffer(@todos)
      File.rm_rf!(dir)
    end)
