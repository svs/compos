defmodule Compos.ListWindowPointSchemeTest do
  @moduledoc """
  Runs priv/tests/list-window-point-test.scm alone: a list redraw keeps every window on its row.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "list-window-point-test.scm"])
  ...
  @tag timeout: 120_000
  test "list-window-point-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
