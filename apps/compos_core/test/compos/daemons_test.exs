defmodule Compos.DaemonsTest do
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      Compos.Core.kill_buffer("*workspaces*")
      Compos.Core.kill_buffer("*workspace-source*")
      File.rm(path)
      eval!("(daemon-register-current! #f)")
    end)
  ...
  test "C-x d opens the daemon table and RET requests same-tab navigation" do
    ...
    assert eval!(~s{(buffer-text "*daemons*")}) =~ "CURRENT  compos · workspace"
  ...
  test "a stopped worktree daemon starts before the tab switches" do
    ...
    File.mkdir_p!(Path.join(workspace, "apps/compos_core"))
    File.write!(Path.join(workspace, "mix.exs"), "# test checkout\n")
    test = self()

    Application.put_env(:compos_core, :workspace_daemon_provisioner, fn path, name ->
  ...
  test "a worktree switch stays put when its daemon cannot start" do
    ...
    Application.put_env(:compos_core, :workspace_daemon_provisioner, fn _path, _name ->
      {:error, :boot_failed}
    end)
  ...
  test "C-x w lists workspaces and RET opens the selected daemon in a new tab" do
    ...
    eval!(~s{(daemon-name-workspace! "#{workspace}" "compos" "workspace prompts")})
    ...
    assert eval!(~s{(buffer-text "*workspaces*")}) =~ "compos"
