...
defmodule Compos.ChatAgentTest do
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      Compos.Core.kill_buffer("*zz-uchat*")
    end)
  ...
  test "a chat becomes an agent thread; session/new carries the caller's MCP set" do
    ...
    assert %{"name" => "COMPOS_AGENT", "value" => slug} in zz["env"]
  ...
  test "execute* gives a spawned chat an explicit working directory" do
    directory = "/tmp/compos-explicit-workspace/"
    slug =
      String.trim(
  ...
  test "a failed inline turn says why and clears its pending send" do
    ...
    assert eval!(~s{(begin (list-refresh! "*Messages*") (buffer-text "*Messages*"))}) =~ "LLM failed · model refused"
  end
  ...
  test "an inline send answers the permission its own tool call raises" do
    ...
    event =
      ~s{(list 'type 'permission 'rpc-id 92 'title "Use compos/apropos" 'kind "mcp" 'options } <>
