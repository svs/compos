defmodule Compos.HelpTest do
  ...
  setup do
    ...
    on_exit(fn ->
      for b <- ["*Help*", "*switch*", "*zz-help*"], do: Compos.Core.kill_buffer(b)
      Editor.delete_other_windows()
    end)
  ...
  test "C-h m in the switcher opens the mode's page, rendered and read-only" do
    ...
    assert text =~ "| `RET` | [`switch-visit`](compos:cmd/switch-visit) |"
  ...
  test "the binding help lists local bindings before global ones" do
    ...
    assert this =~ "compos:cmd/", "the local table listed no command"
    assert every =~ "compos:cmd/", "the global table listed no command"
  ...
  test "C-h k describes the next key instead of running it" do
    ...
    text = Buffer.text("*Help*")
    assert text =~ "# `C-x C-f`"
    assert text =~ "**[`find-file`](compos:cmd/find-file)** — Visit a file"
  ...
  test "a name in a help page is a link to its source, and the link opens it" do
    ...
    assert Buffer.text("*Help*") =~ "**[`find-file`](compos:cmd/find-file)**"

    # following it lands in the file that defines find-file, at the form
    eval!(~s{(preview-follow-link! (active-window) "compos:cmd/find-file")})
  ...
  test "M-? describes the command named at point, with the key that runs it" do
    ...
    assert text =~ "## [`split-window-right`](compos:cmd/split-window-right) — a command"
  ...
  test "M-? describes a public function at point by its signature" do
    ...
    press("M-?")
    assert Buffer.text("*Help*") =~ "## [`help-doc!`](compos:def/help-doc%21) — a function"
