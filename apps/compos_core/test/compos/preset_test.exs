...
defmodule Compos.PresetTest do
  ...
  tools; the core compos editor bridge is intrinsic. An ACP session fixes
  ...
  test "loading a preset refreshes an API chat's frozen tool surface" do
    ...
    # compos is intrinsic, so selecting it need not materialize a redundant
  ...
  test "ACP-backed llm-mode mounts the buffer presets as MCP servers" do
    ...
    assert Map.has_key?(mcp_servers, "compos")
    ...
    # The compos MCP bridge asks this boolean question before eval-scheme.
  ...
  test "loading a preset on a live ACP chat reattaches; the conversation survives" do
    ...
    assert servers(first_new) == ["compos"]
    ...
    assert Buffer.get_local(buf, "chat-presets") == [sym: "zz-pack", sym: "compos"]
    ...
    assert Enum.sort(servers(np2)) == ["compos", "zz-weather"]
  ...
  test "presets declared at spawn survive a revive: they land in the buffer-local" do
    ...
    buf = Enum.find(Compos.Core.list_buffers(), &(Buffer.get_local(&1, "agent-slug") == "a1"))
  ...
  test "chat-tool-surface names every server the chat's agent holds, and no other" do
    ...
    on_exit(fn -> Compos.Core.kill_buffer("*chat servers*") end)
  ...
  test "the compos editor bridge is intrinsic even for an empty restored chat" do
    buf = "zz-intrinsic-compos-#{System.unique_integer([:positive])}"
    on_exit(fn -> if Buffer.exists?(buf), do: Compos.Core.kill_buffer(buf) end)
    ...
    assert servers =~ ~s{"compos"}
    assert servers =~ "compos-mcp-proxy.exs"
  end
