defmodule Compos.Core.Hotload.Compile do
  @moduledoc """
  ...
  "module is not available". `Compos.Core.Editor` died that way twice on
  ...
  defp reload(mod, file) do
    if :code.purge(mod),
      do: Logger.warning("Compos.Core.Hotload: killed processes still running an old #{inspect(mod)}")
    ...
    case :code.load_abs(String.to_charlist(Path.rootname(file))) do
      ...
      {:error, reason} ->
        Logger.error("Compos.Core.Hotload: could not load #{inspect(mod)}: #{inspect(reason)}")
        :error
    end
  end
