;;; chrome.scm --- compos's keys and commands in every tab, and the wire back.
;;;
...
;;; command table — not a copy — so anything you can run in compos you can run
;;; from a page.
;;;
...
;;;
;;; Inside compos, C-x b pulls a buffer into the selected window — Emacs.
...
;; A page draws prompts, not editor windows. Use the same group switcher
;; that C-x b opens inside the editor.
(define *chrome-chord-commands*
  '(("C-x b" "group-switch-buffer")))
...
(define (chrome--mb-key spec)
  ...
  ;; read it, so picking a tab with compos's OWN C-x b left it #f with nothing
...
;; K gets one plist per compos tab the browser holds: window, tab, frame.
...
(public! 'browser-frames
  "(browser-frames K) — K gets one plist per compos browser tab: window, tab, frame")
