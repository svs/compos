defmodule Compos.CommandSchemeTest do
  @moduledoc """
  Runs priv/tests/command-test.scm alone: interactive specs, prefix counts, kill appending, undo boundaries.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "command-test.scm"])
  ...
  @tag timeout: 120_000
  test "command-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
