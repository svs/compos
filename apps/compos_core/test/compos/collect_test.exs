defmodule Compos.CollectTest do
  @moduledoc """
  ...
  setup do
    ...
    on_exit(fn ->
      Editor.minibuffer_close()
      for b <- ["*zz-src*", "*Collect*"], do: Compos.Core.kill_buffer(b)
      Editor.delete_other_windows()
    end)
  ...
  test "C-c C-o collects the candidates and previews the first one" do
    ...
    text = Compos.Core.Buffer.text("*Collect*")
    assert text =~ "alpha"
    assert text =~ "beta"
    assert text =~ "gamma"
  ...
  test "n and p preview, RET confirms the line" do
    ...
    refute Compos.Core.Buffer.exists?("*Collect*")
  end
