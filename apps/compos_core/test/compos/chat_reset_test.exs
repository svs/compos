...
defmodule Compos.ChatResetTest do
  ...
  test "C-c m on an ACP chat reconnects on the new model" do
    ...
    Enum.each(String.graphemes("claude-opus-5"), &Compos.Core.KeyDispatch.handle_key/1)
    Compos.Core.KeyDispatch.handle_key("RET")
  ...
  test "switching backends never rebinds keys: RET is agent-send on every lane" do
    ...
    assert Compos.Core.Editor.lookup_key(["RET"]) == {:command, "agent-send"}

    eval!(~s{(run-command "chat-set-backend")})
    Compos.Core.KeyDispatch.handle_key("b")
    Enum.each(String.graphemes("api"), &Compos.Core.KeyDispatch.handle_key/1)
    Compos.Core.KeyDispatch.handle_key("RET")
    Compos.Core.KeyDispatch.handle_key("m")
    Enum.each(String.graphemes("openai:gpt-5.6-luna"), &Compos.Core.KeyDispatch.handle_key/1)
    Compos.Core.KeyDispatch.handle_key("RET")
    Compos.Core.KeyDispatch.handle_key("e")
    Enum.each(String.graphemes("high"), &Compos.Core.KeyDispatch.handle_key/1)
    Compos.Core.KeyDispatch.handle_key("RET")
    Compos.Core.KeyDispatch.handle_key("C-g")
  ...
  describe "the locals partition (W8)" do
    ...
    setup do
      ...
      on_exit(fn ->
        Application.delete_env(:compos_core, :acp_transport)
        Enum.each(Compos.Core.Agent.list(), &Compos.Core.Agent.kill/1)
    ...
    defp assert_clean(buf) do
      ...
      for k <- ~w(chat-wire-turns chat-turns agent-folds chat-cost chat-last-usage chat-queued) do
        assert Compos.Core.Buffer.get_local(buf, k) in [nil, false, []],
               "#{k} survived the reset: #{inspect(Compos.Core.Buffer.get_local(buf, k))}"
      end

      # ...blocks hold nothing but the freshly rebuilt meta card: no user
      # turn, no prose, no tool card, and above all no stale banner
      kinds =
        (Compos.Core.Buffer.get_local(buf, "agent-blocks") || [])
      ...
      assert Compos.Core.Buffer.get_local(buf, "agent-saved-mark")
      assert Compos.Core.Buffer.text(buf) =~ "you: "
    ...
    test "reset leaves the same clean state from every starting point" do
      ...
      assert_clean(a)
      # identity survived
      assert Compos.Core.Buffer.get_local(a, "agent-connector") == "api"
      ...
      assert_receive {:transport_open, _}, 1_000
      d = "*chat:a1*"
      slug = Compos.Core.Buffer.get_local(d, "agent-slug")
      assert slug in Compos.Core.Agent.list()

      eval!(~s[(begin (switch-to-buffer! "#{d}") (run-command "chat-reset") #t)])
      assert_clean(d)
      # the thread itself is gone, not just forgotten by the buffer
      refute slug in Compos.Core.Agent.list()

      # 5. reset o reset = reset (idempotent)
      before = Compos.Core.Buffer.text(d)
      eval!(~s[(begin (switch-to-buffer! "#{d}") (run-command "chat-reset") #t)])
      assert Compos.Core.Buffer.text(d) == before
      assert_clean(d)
    end
    ...
    test "a restored chat sheds its dead runtime state; a live one keeps it" do
      ...
      on_exit(fn -> Compos.Core.kill_buffer(r) end)

      assert Compos.Core.Buffer.get_local(r, "agent-slug") in [nil, false]
      assert Compos.Core.Buffer.get_local(r, "agent-queued") in [nil, false]
      # a queued message the dead runtime never read returns to the input
      assert Compos.Core.Buffer.get_local(r, "chat-queued") in [nil, false]
      assert eval!(~s{(chat-input-text "#{r}")}) == ~s{"held for the dead runtime"}
      # ...but what was SAID and who the chat IS both survive
      assert Compos.Core.Buffer.get_local(r, "chat-wire-turns") != nil
      assert Compos.Core.Buffer.get_local(r, "agent-connector") == "codex"
      ...
      assert Compos.Core.Buffer.get_local(live, "agent-slug") == "a1"
    end
