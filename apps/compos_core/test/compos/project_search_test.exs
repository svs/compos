defmodule Compos.ProjectSearchTest do
  @moduledoc """
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      for f <- ["lib/a.txt", "lib/b.txt", "top.txt"],
          do: Compos.Core.kill_buffer(Path.join(root, f))

      Compos.Core.kill_buffer("*zz-ps*")
      Compos.Core.kill_buffer(root)
      Editor.delete_other_windows()
      File.rm_rf!(root)
    end)
  ...
  describe "project-find-file candidates" do
    ...
    test "a file git does not know still opens", %{root: root} do
      ...
      Compos.Core.kill_buffer("#{root}/lib/brand-new.txt")
    end
