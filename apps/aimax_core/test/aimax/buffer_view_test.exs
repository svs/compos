defmodule Aimax.BufferViewTest do
  @moduledoc """
  The buffer read model (`Aimax.Core.BufferView`).
  ...
  alias Aimax.Core.{Buffer, BufferStore, BufferView, Editor}

  setup do
    name = "*view-#{System.unique_integer([:positive])}*"
    {:ok, ^name} = Aimax.Core.create_buffer(name, text: "hello\nworld\n")
  ...
  describe "the row follows the buffer" do
    ...
    test "the row is current before the change event announces it", %{name: name} do
      ...
      Aimax.Core.Events.subscribe(name)
      ...
      Aimax.Core.Events.unsubscribe(name)
  ...
  describe "the row's lifetime" do
    test "a rename moves the row and leaves nothing behind", %{name: name} do
      ...
      on_exit(fn -> Aimax.Core.kill_buffer(renamed) end)
    ...
    test "a dead buffer keeps no row, and reads fall back to its checkpoint" do
      ...
      {:ok, ^name} = Aimax.Core.create_buffer(name, text: "persisted")
    ...
    test "a crash of the model heals, and no buffer dies with it", %{name: name} do
      [{buffer, _}] = Registry.lookup(Aimax.Core.BufferRegistry, name)
      ...
      {:ok, ^other} = Aimax.Core.create_buffer(other, text: "x")
