defmodule Compos.Core.Remote do
  @moduledoc """
  ...
  def ssh, do: Application.get_env(:compos_core, :ssh_cmd, "ssh")
  ...
  def write(host, path, text) do
    ...
    tmp = Path.join(System.tmp_dir!(), "compos-remote-#{System.unique_integer([:positive])}")
    File.write!(tmp, text)
